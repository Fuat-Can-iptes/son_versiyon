...
// Veritabanı Gereksinimi: Araçları, kargo firmaları ve alt tipleriyle (Kamyon/Motor) birleştirerek çekiyoruz.
$sorgu = $db->query("
    SELECT 
        v.*, 
        ...
        m.BoxVolume
    FROM vehicles v 
    LEFT JOIN shippers s ON v.ShipperId = s.Id
    LEFT JOIN trucks t ON v.Id = t.VehicleId
    LEFT JOIN motorcycles m ON v.Id = m.VehicleId
    ORDER BY v.Id DESC
");
...
<head>
    ...
    <style>
        ...
        .badge-kamyon { background: #e0f2fe; color: #0284c7; padding: 4px 8px; border-radius: 6px; font-weight: bold; font-size: 12px; }
        .badge-motor { background: #ffedd5; color: #ea580c; padding: 4px 8px; border-radius: 6px; font-weight: bold; font-size: 12px; }
    </style>
</head>
<body>
    ...
    <main class="admin-container">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
            <div>
                ...
                <p style="color: #64748b;">Teslimat filonuzdaki araçların plaka, marka ve kapasite kayıtları.</p>
            </div>
            ...
            <table class="modern-table">
                <thead>
                    <tr>
                        <th>Plaka</th>
                        <th>Marka / Model</th>
                        <th>Tip ve Kapasite</th> <th>Bağlı Firma</th>
                        <th>Durum</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if(empty($araclar)): ?>
                        <tr><td colspan="5" style="text-align: center; padding: 30px; color: #94a3b8;">Henüz kayıtlı araç bulunmuyor.</td></tr>
                    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if($a['Vehicle_Type'] == 'K'): ?>
                                    <span class="badge-kamyon"><i class="fa-solid fa-truck"></i> Kamyon</span>
                                    <div style="margin-top: 5px; font-size: 12px; color: #64748b;">Tonaj: <b><?php echo htmlspecialchars($a['MaxWeight']); ?> Ton</b></div>
                                <?php elseif($a['Vehicle_Type'] == 'M'): ?>
                                    <span class="badge-motor"><i class="fa-solid fa-motorcycle"></i> Motosiklet</span>
                                    <div style="margin-top: 5px; font-size: 12px; color: #64748b;">Hacim: <b><?php echo htmlspecialchars($a['BoxVolume']); ?> Litre</b></div>
                                <?php endif; ?>
                            </td>

                            <td style="color: #64748b;"><?php echo htmlspecialchars($a['ShipperName'] ?? 'Firma Yok'); ?></td>
